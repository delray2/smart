...
// MARK: - Roborock Authentication Manager
class RoborockAuthManager: ObservableObject, PlatformAuthManager {
    private let authURL = "https://account.roborock.com/oauth2/authorize"
    private let tokenURL = "https://account.roborock.com/oauth2/token"
    private let baseURL = "https://api.roborock.com/v1"
    
    // TODO: Replace with your actual credentials from Roborock Developer Portal
    private let clientId = "YOUR_ROBOROCK_CLIENT_ID"
    private let clientSecret = "YOUR_ROBOROCK_CLIENT_SECRET"
    private let redirectURI = "roomscanningapp://oauth/roborock"
    ...
    private func exchangeCodeForTokens(authCode: String) async throws -> (accessToken: String, refreshToken: String, expiresAt: Date) {
        ...
        let tokenResponse = try JSONDecoder().decode(RoborockTokenResponse.self, from: data)
    ...
    func discoverDevices(credentials: PlatformCredentials) async throws -> [PlatformDevice] {
        ...
        let devicesResponse = try JSONDecoder().decode(RoborockDevicesResponse.self, from: data)
    ...
    private func buildCapabilities(from device: RoborockDevice) -> [String] {
        var capabilities: [String] = ["vacuum", "navigation", "mapping"]
        
        if device.capabilities.contains("mop") {
            capabilities.append("mop")
        }
        
        if device.capabilities.contains("camera") {
            capabilities.append("camera")
        }
        
        if device.capabilities.contains("selfEmpty") {
            capabilities.append("selfEmpty")
        }
        
        if device.capabilities.contains("selfWash") {
    ...
    func executeAction(_ action: DeviceAction, on device: PlatformDevice, credentials: PlatformCredentials, parameters: [String: Any]?) async throws {
        ...
        guard let url = URL(string: "\(baseURL)/devices/\(device.id)/control") else {
        ...
        let command = buildRoborockCommand(action: action, parameters: parameters)
    ...
    private func buildRoborockCommand(action: DeviceAction, parameters: [String: Any]?) -> [String: Any] {
        switch action {
        case .startCleaning:
            return [
                "command": "start",
    ...
    func getDeviceStatus(_ device: PlatformDevice, credentials: PlatformCredentials) async throws -> DeviceStatus {
        ...
        guard let url = URL(string: "\(baseURL)/devices/\(device.id)/status") else {
        ...
        let statusResponse = try JSONDecoder().decode(RoborockStatusResponse.self, from: data)
    ...
    func authenticate(with apiKey: String) async throws -> PlatformCredentials {
        throw PlatformError.authenticationFailed("API key authentication not supported for Roborock.")
    }
    
    func authenticate(with token: String, hubIP: String) async throws -> PlatformCredentials {
        throw PlatformError.authenticationFailed("Token/hubIP authentication not supported for Roborock.")
    }
...
// MARK: - Roborock API Models
struct RoborockTokenResponse: Codable {
...
struct RoborockDevicesResponse: Codable {
    let devices: [RoborockDevice]
}

struct RoborockDevice: Codable {
    let deviceId: String
    let name: String
    let model: String
    let isOnline: Bool
...
struct RoborockStatusResponse: Codable {
    let status: RoborockStatus
}

struct RoborockStatus: Codable {
    let isOnline: Bool
    let state: String
    let battery: Int?
